-- Kanban System Database Schema for Render.com
-- PostgreSQL
-- Note: Database already exists, so we skip CREATE DATABASE and \c commands
...
CREATE INDEX IF NOT EXISTS idx_workspace_members_is_deleted ON workspace_members(is_deleted);
...
CREATE INDEX IF NOT EXISTS idx_board_members_is_deleted ON board_members(is_deleted);
...
CREATE INDEX IF NOT EXISTS idx_lists_is_deleted ON lists(is_deleted);
